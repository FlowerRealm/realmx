...
mod tests {
    ...
    #[test]
    fn create_seatbelt_args_with_read_only_git_and_realmx_subpaths() {
        // Create a temporary workspace with two writable roots: one containing
        // top-level .git and .realmx directories and one without them.
        let tmp = TempDir::new().expect("tempdir");
        let PopulatedTmp {
            vulnerable_root,
            vulnerable_root_canonical,
            dot_git_canonical,
            dot_realmx_canonical,
            empty_root,
        ...
        // write to .realmx/config.toml in the vulnerable root.
        ...
        // - write access to WRITABLE_ROOT_0 (but not its .git or .realmx), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
"#,
        ...
        let mut expected_args = vec![
            ...
            format!(
                ...
                dot_realmx_canonical.to_string_lossy()
            ),
            format!(
                "-DWRITABLE_ROOT_1={}",
        ...
        // Verify that .realmx/config.toml cannot be modified under the generated
        // Seatbelt policy.
        let config_toml = dot_realmx_canonical.join("config.toml");
        ...
        // Verify that writing a file to the folder containing .git and .realmx is allowed.
    ...
    fn create_seatbelt_args_for_cwd_as_git_repo() {
        ...
        // `.realmx` checks are done properly for cwd.
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            network_access: false,
...
"#,
    ...
    struct PopulatedTmp {
        /// Path containing a .git and .realmx subfolder.
        ...
        /// ran `git commit` themselves, or modified .realmx/config.toml to
        ...
        dot_realmx_canonical: PathBuf,

        /// Path without .git or .realmx subfolders.
        empty_root: PathBuf,
    ...
    fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
        ...
        fs::create_dir_all(vulnerable_root.join(".realmx")).expect("create .realmx");
        fs::write(
            vulnerable_root.join(".realmx").join("config.toml"),
            "sandbox_mode = \"read-only\"\n",
        )
        .expect("write .realmx/config.toml");
        ...
        let dot_realmx_canonical = vulnerable_root_canonical.join(".realmx");
