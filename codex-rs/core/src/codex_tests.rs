...
fn test_model_client_session() -> crate::client::ModelClientSession {
    crate::client::ModelClient::new(
        ...
        crate::OAuthCredentialsStoreMode::File,
...
fn test_active_plan_items() -> Vec<ThreadPlanItemCreateParams> {
    vec![ThreadPlanItemCreateParams {
        row_id: "plan-01".to_string(),
        row_index: 0,
        status: ThreadPlanItemStatus::InProgress,
        step: "Keep plan fresh".to_string(),
        path: "codex-rs/core/src/codex.rs".to_string(),
        details: String::new(),
        inputs: Vec::new(),
        outputs: Vec::new(),
        depends_on: Vec::new(),
...
async fn seed_active_plan(
    session: &Session,
    state_db: &codex_state::StateRuntime,
    source_turn_id: &str,
) {
    let raw_csv = codex_state::render_thread_plan_csv(test_active_plan_items().as_slice())
        .expect("render active plan csv");
    state_db
        .replace_active_thread_plan(&ThreadPlanSnapshotCreateParams {
            id: format!("snapshot-{source_turn_id}"),
            thread_id: session.conversation_id.to_string(),
            source_turn_id: source_turn_id.to_string(),
            source_item_id: format!("item-{source_turn_id}"),
            raw_csv,
        })
        .await
...
async fn on_task_finished_keeps_active_plan_for_matching_turn() {
    ...
    seed_active_plan(&session, state_db.as_ref(), turn_context.sub_id.as_str()).await;
...
#[tokio::test]
async fn on_task_finished_does_not_clear_other_turns_active_plan() {
    let codex_home = tempfile::tempdir().expect("create temp codex home");
    let state_db = codex_state::StateRuntime::init(
        codex_home.path().to_path_buf(),
        "test-provider".to_string(),
    ...
    let (mut session, turn_context) = make_session_and_context().await;
    session.services.state_db = Some(Arc::clone(&state_db));
    seed_active_plan(&session, state_db.as_ref(), "other-turn").await;
    ...
    let active_plan = state_db
        .get_active_thread_plan(session.conversation_id.to_string().as_str())
        .await
        .expect("load active plan after unrelated turn completion")
        .expect("active plan should remain");
    assert_eq!(active_plan.snapshot.source_turn_id, "other-turn");
}
