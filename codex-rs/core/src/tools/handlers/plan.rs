...
pub(crate) async fn handle_update_plan(
    ...
    let event_args = try_update_active_thread_plan(session, &args)
        .await?
        .unwrap_or(args);
    session
        .send_event(turn_context, EventMsg::PlanUpdate(event_args))
        .await;
    Ok(PLAN_UPDATED_MESSAGE.to_string())
}
...
fn parse_update_plan_arguments(arguments: &str) -> Result<UpdatePlanArgs, FunctionCallError> {
    serde_json::from_str::<UpdatePlanArgs>(arguments).map_err(|err| {
...
async fn try_update_active_thread_plan(
    ...
    match classify_active_plan_update(args, &active_plan)? {
        ...
        ActivePlanUpdate::Replace(items) => {
            ...
            let raw_plan_markdown = render_plan_csv_markdown(items.as_slice()).map_err(|err| {
                FunctionCallError::RespondToModel(format!(
...
fn plan_item_has_structured_metadata(item: &PlanItemArg) -> bool {
    item.path.is_some()
...
fn active_plan_replacement_items(
    ...
    let mut in_progress_count = 0usize;
    let mut seen_row_ids = HashSet::with_capacity(args.plan.len());
...
fn matching_existing_plan_item<'a>(
    ...
    existing_rows: &HashMap<&str, &'a codex_state::ThreadPlanItem>,
...
mod tests {
    ...
    use super::replacement_snapshot_source;
