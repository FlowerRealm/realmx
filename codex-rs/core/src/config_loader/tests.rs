...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".realmx")).await?;
    tokio::fs::create_dir_all(project_root.join(".realmx")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".realmx").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".realmx").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    assert_eq!(
        ...
        nested.join(".realmx").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".realmx").as_path()
    );
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".realmx").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".realmx").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".realmx").join("child.txt"),
        "child instructions",
    )
    .await?;
