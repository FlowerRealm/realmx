...
mod tests {
    ...
    fn resume_response_replays_active_plan_after_history_turns() {
        ...
        match &initial_messages[0] {
            EventMsg::PlanUpdate(update) => {
                ...
                assert_eq!(update.plan[0].path.as_deref(), Some("a.rs"));
            }
    ...
    #[test]
    fn thread_initial_messages_restore_active_plan_metadata_when_history_exists() {
        let thread_id = ThreadId::new();
        ...
        let initial_messages =
            thread_initial_messages(&thread).expect("thread should restore initial messages");
        ...
        match &initial_messages[0] {
            EventMsg::PlanUpdate(update) => {
                ...
                assert_eq!(update.plan[0].details.as_deref(), Some("latest"));
            }
            other => panic!("expected restored active plan update, got {other:?}"),
        }
    }
