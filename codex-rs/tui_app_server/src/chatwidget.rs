...
impl ChatWidget {
    ...
    pub(crate) fn handle_server_notification(
        ...
        match notification {
            ...
            ServerNotification::SkillUsed(notification) => {
                let invocation_type = match notification.invocation_type {
                    codex_app_server_protocol::SkillInvocationType::Explicit => "explicit",
                    codex_app_server_protocol::SkillInvocationType::Implicit => "implicit",
                };
        ...
        match msg {
            ...
            EventMsg::TurnStarted(event) => {
                self.current_collaboration_mode = CollaborationMode {
                    mode: event.collaboration_mode_kind,
                    plan_phase: event.plan_phase,
                    settings: self.current_collaboration_mode.settings.clone(),
                }
                .normalized();
                self.update_collaboration_mode_indicator();
                self.refresh_model_display();
            ...
            EventMsg::SkillUsed(notification) => {
                let invocation_type = match notification.invocation_type {
                    codex_protocol::protocol::SkillInvocationType::Explicit => "explicit",
    ...
    #[cfg(test)]
    pub(crate) fn active_plan_phase_for_test(&self) -> Option<PlanModePhase> {
        self.active_plan_phase()
    }
    ...
    fn active_mode_kind(&self) -> ModeKind {
        self.effective_collaboration_mode().mode
    }

    fn active_plan_phase(&self) -> Option<PlanModePhase> {
    ...
    pub(crate) fn set_collaboration_mask(&mut self, mut mask: CollaborationModeMask) {
        ...
        let next_plan_phase = self.active_plan_phase();
