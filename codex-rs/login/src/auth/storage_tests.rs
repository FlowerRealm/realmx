...
fn auth_store(auth: AuthDotJson) -> AuthStoreJson {
    AuthStoreJson::from_default(auth)
}

#[tokio::test]
async fn file_storage_load_returns_auth_store_json() -> anyhow::Result<()> {
    ...
    assert_eq!(Some(auth_store_json), loaded);
    Ok(())
}

#[tokio::test]
async fn file_storage_save_persists_auth_store_json() -> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let storage = FileAuthStorage::new(codex_home.path().to_path_buf());
    let auth_dot_json = AuthDotJson {
        auth_mode: Some(AuthMode::ApiKey),
    ...
    let same_auth_store_json = storage
        .try_read_auth_store_json(&file)
        .context("failed to read auth file after save")?;
    assert_eq!(auth_store_json, same_auth_store_json);
    Ok(())
}
...
fn ephemeral_storage_save_load_delete_is_in_memory_only() -> anyhow::Result<()> {
    ...
    assert_eq!(Some(auth_store(auth_dot_json)), loaded);
...
fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
    ...
    seed_keyring_with_auth(
        ...
        &auth_store(expected.clone()),
    )?;

    let loaded = storage.load()?;
    assert_eq!(Some(auth_store(expected)), loaded);
    Ok(())
}
...
fn auto_auth_storage_load_prefers_keyring_value() -> anyhow::Result<()> {
    ...
    let keyring_auth = auth_store(auth_with_prefix("keyring"));
    seed_keyring_with_auth(
        &mock_keyring,
        || compute_store_key(codex_home.path()),
        &keyring_auth,
    ...
    storage.file_storage.save(&auth_store(file_auth))?;
...
fn auto_auth_storage_load_uses_file_when_keyring_empty() -> anyhow::Result<()> {
    ...
    storage.file_storage.save(&auth_store(expected.clone()))?;

    let loaded = storage.load()?;
    assert_eq!(loaded, Some(auth_store(expected)));
    Ok(())
}
...
fn auto_auth_storage_save_prefers_keyring() -> anyhow::Result<()> {
    ...
    storage.file_storage.save(&auth_store(stale))?;

    let expected = auth_with_prefix("to-save");
    storage.save(&auth_store(expected.clone()))?;
...
fn auto_auth_storage_save_falls_back_when_keyring_errors() -> anyhow::Result<()> {
    ...
    assert_eq!(saved, auth_store(auth));
    assert!(
