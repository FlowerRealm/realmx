...
#[clap(
    ...
    // `realmx-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `realmx` command name that users run.
    bin_name = "realmx",
    override_usage = "realmx [OPTIONS] [PROMPT]\n       realmx [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    if let Some(session_id) = conversation_id {
        let resume_cmd = format!("realmx resume {session_id}");
...
async fn cli_main(codex_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Review(review_args)) => {
            let mut exec_cli = ExecCli::try_parse_from(["realmx", "exec"])?;
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | realmx login --with-api-key`."
                        );
                        std::process::exit(1);
...
mod tests {
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["realmx", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["realmx", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["realmx", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["realmx", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["realmx", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["realmx", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["realmx", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["realmx", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["realmx", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["realmx", "app-server"].as_ref());
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["realmx", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }
