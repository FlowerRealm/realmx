...
- `turn/start` — add user input to a thread and begin Codex generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications. For `collaborationMode`, use camelCase wire fields (`planPhase`, `settings.reasoningEffort`, `settings.developerInstructions`); `settings.developerInstructions: null` means "use built-in instructions for the selected mode".
...
- `collaborationMode/list` — list available collaboration mode presets (experimental, no pagination). The preset metadata uses camelCase wire fields such as `planPhase`. This response omits built-in developer instructions; clients should either pass `settings.developerInstructions: null` when setting a mode to use Codex's built-in instructions, or provide their own instructions explicitly.
...
- `turn/plan/updated` — `{ turnId, explanation?, rawCsv, plan }` whenever the agent shares or changes its plan; `rawCsv` is the canonical plan CSV when the update is fully structured, otherwise `null`. Each `plan` entry is `{ id?, step, status, path?, details?, inputs?, outputs?, dependsOn?, acceptance? }` with `status` in `pending`, `inProgress`, or `completed`.
